                            ...
                            <tbody>
                                @foreach ($transactionsperiods as $period => $data)
                                    <tr>
                                        <td>{{ $period }}</td>
                                        <td>{{ $data->count() }}</td>
                                        <td>{{ \MoneyFormat::format_money($data->where('type', 'income')->sum('amount')) }}</td>
                                        <td>{{ \MoneyFormat::format_money($data->where('type', 'expense')->sum('amount')) }}</td>
                                        <td>{{ \MoneyFormat::format_money($data->where('type', 'payment')->sum('amount')) }}</td>
                                        <td>{{ \MoneyFormat::format_money($data->where('payment_method_id', optional($methods->where('name', 'Cash')->first())->id)->sum('amount')) }}</td>
                                        <td>{{ \MoneyFormat::format_money($data->sum('amount')) }}</td>
                                        <td></td>
                                    </tr>
                                @endforeach
                            </tbody>
                            ...
                            <tbody>
                                @foreach($clients as $client)
                                    <tr>
                                        ...
                                        <td>{{ \MoneyFormat::format_money($client->transactions->sum('amount')) }}</td>
                                        <td>
                                            @if ($client->balance > 0)
                                                <span class="text-success">{{ \MoneyFormat::format_money($client->balance) }}</span>
                                            @elseif ($client->balance < 0.00)
                                                <span class="text-danger">{{ \MoneyFormat::format_money($client->balance) }}</span>
                                            @else
                                                {{ \MoneyFormat::format_money($client->balance) }}
                                            @endif
                                        </td>
                                        <td>
                            ...
                            <tbody>
                                @foreach($methods as $method)
                                    <tr>
                                        ...
                                        <td>{{ \MoneyFormat::format_money($transactionsperiods['Year']->where('payment_method_id', $method->id)->count()) }}</td>
                                        <td>{{ \MoneyFormat::format_money($transactionsperiods['Year']->where('payment_method_id', $method->id)->sum('amount')) }}</td>
                                        <td>
                    ...
                    <tbody>
                        @foreach ($salesperiods as $period => $data)
                            <tr>
                                ...
                                <td>{{ \MoneyFormat::format_money($data->avg('total_amount')) }}</td>
                                <td>{{ \MoneyFormat::format_money($data->where('finalized_at', '!=', null)->map(function ($sale) {return $sale->products->sum('total_amount');})->sum()) }}</td>
