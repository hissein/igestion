    ...
    <div class="intro-y grid grid-cols-12 gap-5 mt-5">
        ...
        <div class="intro-y col-span-12 lg:col-span-8">
            ...
            <div class="grid grid-cols-12 gap-5 mt-5">
                @php
                    $active = random_int(0,count($categories))
                @endphp
                @foreach($categories as $key => $category)
                    <div class="col-span-12 sm:col-span-4 2xl:col-span-3 box {{$key === $active ? 'bg-primary ' : ''}} p-5 cursor-pointer zoom-in">
                        <div class="font-medium text-base {{$key === $active ? 'text-white' : ''}}">{{$category->name}}</div>
                        <div class="text-slate-500">{{$category->products_count}} Produits</div>
                    </div>
                @endforeach
                        ...
                        <div class="box rounded-md p-3 relative zoom-in">
                            <div class="flex-none relative block before:block before:w-full before:pt-[100%]">
                                <div class="absolute top-0 left-0 w-full h-full image-fit">
                                    <img alt="Tinker Tailwind HTML Admin Template" class="rounded-md" src="{{count($product->assets) > 0 ? $product->assets[0]->name : '' }}">
                                </div>
                            </div>
